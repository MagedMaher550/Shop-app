...
userSchema.methods.addToCart = function(product) {
  ...
  const oldTotalPrice = parseFloat((this.cart.totalPrice ? this.cart.totalPrice : 0));
  let updatedTotalPrice = oldTotalPrice;

  if(cartProductIndex >= 0) {
      newQuantity = this.cart.items[cartProductIndex].quantity + 1;
      updatedCartItems[cartProductIndex].quantity = newQuantity;
      updatedTotalPrice += parseFloat(product.price);
  } else {
...
userSchema.methods.deleteItemFromCart = function(prodId) {
  ...
  return Product.findById(prodId)
    .then(product => {
      ...
      const updatedTotalPrice = (oldTotalPrice - (productPrice * productQuantity)).toFixed(2);
